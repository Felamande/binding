...
func (v ValidateAction) Post() error {
...
func performValidationTest(t *testing.T, testCase validationTestCase) {
	...
	obj = testCase.data
	m.Post(testRoute, new(ValidateAction))
	...
	case http.StatusNotFound:
		if testCase.expectedErrors.Len() == 0 {
	...
	So(fmt.Sprintf("%+v", errors), ShouldEqual, fmt.Sprintf("%+v", testCase.expectedErrors))
}
